...
export default function Page() {
  ...
  const centres = [
    {
      ...
      description: `Welcome, we hope you enjoy it here`,
      location: "2.5 miles",
    },
    {
      id: 2,
  ...
  const handlePress = (centre) => {
    setModalVisible(true);
    setCentreName(centre);
  };
  const onPress = (page) => {
    router.push(`/${page}?centre=${centreName}`);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <AppCard
            ...
            handlePress={() => handlePress(item.centreName)}
          />
        )}
      />
